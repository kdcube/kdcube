 ...
 *
 * EMIT (an Agent Card affordance that prefers a scene window): post a
 ...
 *
 * RECEIVE (the served `capabilities` widget): parse the routed command, apply
 * `{agent_id?, conversation_id?, spotlight_tools?, section?}` at runtime,
 * and ack for host diagnostics. A conversation id keeps an expansion on the
 * same conversation-scoped selection; an omitted id opens Agent Card defaults.
 */
...
export interface CapabilitiesOpenPayload {
  ...
  agent_id?: string
  /** The conversation selection an expanded composer picker should manage. */

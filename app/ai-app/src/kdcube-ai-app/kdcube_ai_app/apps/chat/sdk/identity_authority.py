...
async def resolve_platform_authority(
    ...
    *,
    ...
    roles = _list(getattr(session, "roles", None) or [])
    permissions = _list(getattr(session, "permissions", None) or [])
    authority.update(
        {
            "platform_roles": roles,
            "platform_permissions": permissions,
            "economics_budget_bypass": authority_has_platform_privilege(roles),
...
def authority_from_source(source: Dict[str, Any] | None) -> Dict[str, Any]:
    ...
    return {
        ...
        "budget_bypass": (
            src.get("economics_budget_bypass")
            if isinstance(src.get("economics_budget_bypass"), bool)
            else src.get("budget_bypass")
        ),
        "source": src,
    }
...
def normalize_execution_authority(
    ...
    *,
    ...
    for legacy_key in ("user_type", "platform_user_type", "economics_user_type"):
    ...
    if resolved_roles:
        ...
        out.setdefault("economics_budget_bypass", authority_has_platform_privilege(resolved_roles))

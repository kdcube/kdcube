...
An action that failed for a missing prerequisite (an access approval, a connection, a tool, a file) stays failed after the prerequisite is satisfied — satisfying a prerequisite never turns a past failure into a success. When the prerequisite is now satisfied and the goal is still current in this conversation, re-execute the action; when the blocked attempt is stale and the conversation has moved on, leave it and ask instead of re-firing it.

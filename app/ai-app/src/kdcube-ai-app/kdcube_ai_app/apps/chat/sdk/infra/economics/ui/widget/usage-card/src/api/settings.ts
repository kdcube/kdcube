/**
 * Runtime config + auth header resolver for the usage-card widget.
 *
 * The build step replaces `{{...}}` placeholders with deploy-time values.
 * In dev / placeholder-only mode the widget asks the parent window via
 * `CONFIG_REQUEST` postMessage for runtime config; the scene host (or any
 * embedder) replies with `CONFIG_RESPONSE` carrying base URL + tokens +
 * tenant/project. A `/api/cp-frontend-config` GET is tried first as a
 * lightweight fallback for standalone direct-load.
 */
...
function routeContext() {
  ...
  if (!marker || index < 0) {
    return {
      ...
      widgetAlias: query.get('widget') || 'usage_card',
    };
  }
  ...
  return {
    ...
    widgetAlias: widgetsIndex >= 0 ? parts[widgetsIndex + 1] || 'usage_card' : 'usage_card',
  };
}
...
class Settings {
  ...
  setupParentListener(): Promise<boolean> {
    ...
    window.addEventListener('message', (event: MessageEvent) => {
      ...
      if (event.data.identity !== 'USAGE_CARD_WIDGET' || !event.data.config) return;
    ...
    return new Promise((resolve) => {
      ...
      const requestParentConfig = () => {
        ...
        // Give the parent a few seconds to respond; widget still renders

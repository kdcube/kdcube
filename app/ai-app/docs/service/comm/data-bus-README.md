...
| `subject` | Domain event or command name, such as `task_tracker.canvas.patch`. |
| `object_ref` | Optional object partition key, such as `canvas:main` or `issue:BUG-123`. |
...
async def handle_canvas_patch(ctx, message):
    ...
```
...
For collaborative objects such as canvas state, the durable storage remains the
...
async def handle_canvas_patch(ctx, message):
    result = await ctx.bundle.canvas.apply_patch(
        actor=message.actor,
...
```python
await ctx.reply.ok({
    "type": "task_tracker.canvas.patch.applied",
    "canvas": result.canvas_summary,
})
```
...
In the task tracker bundle:

- creating a user text card on a canvas is a Data Bus message;
- uploading a file to the canvas is a Data Bus message plus bundle hosting;
- dragging a card on the canvas is a Data Bus patch if the bundle chooses to
  persist placement immediately;
- focusing a canvas card into chat is a conversation submission because the
  user is asking the assistant to use that context;
- chat paperclip attachments remain conversation `fi:` attachments;
- canvas-hosted user text and user attachments remain bundle `ext:` refs even

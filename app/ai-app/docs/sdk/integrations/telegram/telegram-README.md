...
store of its own, for example a table keyed by update id that holds across
replicas:
...
try:
    ...
    # The webhook will not answer 200: give the claim back so the resend is handled.
    await release_telegram_update(summary.get("update_id"), store=my_claims)
    raise
```

A store implements both `claim_telegram_update(update_id) -> bool` and
`release_telegram_update(update_id)`.

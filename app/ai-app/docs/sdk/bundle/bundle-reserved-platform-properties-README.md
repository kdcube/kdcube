...
| `services.llm.custom` | no default | `BaseEntrypoint` | Locally served models: shared `endpoint`, shared fallback `num_ctx`, and exact-model `model_overrides.<model-tag>.num_ctx` defaults applied onto `Config` custom-model state; enables `provider: custom` routing. A selected model row may override both with `supported_models[].num_ctx`. The selected `role_models` or composer model is sent per request, so this block has no separate `model_name` or model allowlist. Key travels the secrets path: `services.llm.custom.api_key` in `bundles.secrets.yaml` |
...
| `services.llm.custom` | resolved `Config.custom_model_endpoint` / context settings | `<prefix>/bundles/<bundle_id>/descriptor` `config.services.llm.custom` | `bundles.yaml -> items[].config.services.llm.custom` | cache | platform-owned shared locally-served-models endpoint and serving overrides; model identity comes from role/composer selection |

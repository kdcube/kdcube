...
| App/domain artifact refs visible to an agent | `@artifact_namespace_rehoster(...)` for compact owner namespaces such as `nmsp:...`, `mem:...`, or `cnv:...`; an adapter such as `react.pull` returns the materialized `conv:fi:` path | [App Events](../bundle-events-README.md), [Artifact Resolution And Materialization](../../../runtime/harness/events/artifact-resolution-and-materialization-README.md), [ReAct Event Source](../../agents/react/event-source/event-source-README.md) |
...
| ReAct-side artifact recovery, search, and precise text editing | `react.pull`, `react.checkout`, `react.rg`, `react.read`, `react.patch` | [Agent Harness Workspace](../../../runtime/harness/workspace/README.md), [ReAct Runtime Configuration](../../agents/react/runtime-configuration-README.md) |
...
| Agent capability defaults and conversation selection (tools/skills/MCP/namespace operation-action toggles + model pick) | `agent_capabilities` / `agent_selection_update` on the SDK entrypoint base + `apply_user_agent_selection` in the workflow; chat edits remain a draft until **Save changes** and then update only that conversation within the live Control Card. The same picker body powers the served `capabilities` widget (`sdk://solutions/chat/ui/widget-capabilities`), whose unscoped mode edits Agent Card defaults for future conversations | [Agent Capability Control And Selection](../../solutions/user-settings/capabilities-README.md), [How To Construct A ReAct Agent](../../agents/react/how/how-to-construct-react-agent-README.md) |

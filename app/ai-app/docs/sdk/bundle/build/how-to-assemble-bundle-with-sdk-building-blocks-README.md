...
| Conversation-scoped agent customization (tools/skills/MCP/namespace operation-action toggles + model pick) | `agent_capabilities(conversation_id)` / `agent_selection_update(conversation_id)` on the SDK entrypoint base + `apply_user_agent_selection` in the workflow; chat edits remain a draft until **Save changes** and then narrow only that conversation. The same picker body also powers the served `capabilities` widget (`sdk://solutions/chat/ui/widget-capabilities`); unscoped mode shows the Agent Card base read-only because Connection Hub owns its edits | [Conversation-Scoped Agent Capabilities](../../solutions/user-settings/capabilities-README.md), [How To Construct A ReAct Agent](../../agents/react/how/how-to-construct-react-agent-README.md) |
